      ...
      <div class="wrapper addInfo">
        <ul>
          <li>
            <div class="display_box">
              <span>客户联系电话2：</span>
              <input class="phone" type="text" v-model.trim="form.phone">
            </div>
          </li>
          <li>
            <div class="display_box">
              <span>客户地址：</span>
              <div class="address">
                <el-select class="selectAddress" v-model.trim="form.province" @change="changeProvince">
                  <el-option :label="province" :value="province" v-for="province in provincesArray" :key="province"></el-option>
                </el-select>
                <el-select class="selectAddress" v-model.trim="form.city" @change="changeCity">
                  <el-option :label="city" :value="city" v-for="city in citysArray" :key="city"></el-option>
                </el-select>
                <el-select class="selectAddress" v-model.trim="form.county">
          ...
          <li>
            <div class="display_box">
              ...
              <div class="detailsAddress">
                <textarea placeholder="详细地址" v-model.trim="form.detailsAddress"></textarea>
              </div>
            </div>
          </li>
          <li class="needCon">
            <div class="display_box">
              <span>项目说明：</span>
              <textarea v-model.trim="form.notes"></textarea>
            </div>
          </li>
  ...
  export default {
    ...
    methods: {
      ...
      confirm () {
        this.axios({
          method: 'POST',
          url: '/jingtian/demand/handle',
          data: this.toFormData({
            demandId: this.demandId,
            contact2: this.form.phone,
            province: this.form.province,
            city: this.form.city,
            county: this.form.county,
            address: this.form.detailsAddress,
            introduction: this.form.notes
          })
        }).then(result => {
          if (result.data.code === 0) {
            this.$message.success('需求单处理完成！')
            this.isShowPopup = false
            this.getDatas()
          }
        })
